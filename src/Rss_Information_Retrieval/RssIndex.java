...
public class RssIndex 
{
    ...
    private void RssFinder(File temp) throws IOException, IllegalArgumentException, FeedException
    {
        ...
        try {
            ...
            for (Iterator i = feed.getEntries().iterator(); i.hasNext();) {
                ...
                CalculateFrequencies(tokenizer, temp, 3);
                
                String desc = entry.getDescription().getValue();
                tokenizer = new StringTokenizer(desc, delimiter);
                CalculateFrequencies(tokenizer, temp, 0.5);
        ...
        {
            System.err.println("Bad xml file: "+temp.getName());
        } 
    }
    private void CalculateFrequencies(StringTokenizer tokenizer,File temp,double weight)
    {
            ...
            while(tokenizer.hasMoreTokens())
            {
                ...
                if (!stopSet.contains(currentToken))
                {
                    ...
                    {
                        tf_map.get(currentToken).put(temp,(short) (1*weight));
                    }
                    else
                    {
                        short x=tf_map.get(currentToken).get(temp);
                        tf_map.get(currentToken).put(temp, (short)(++x*weight));
                    ...
                    positions_map.get(currentToken).get(temp).add(feed_number);
                }
            }
    }
    public void GenerateFiles(SortedMap<String,Short> sortedmap)
    {
        ...
        {     
            ...
            FileWriter fstreamP;
            BufferedWriter outP;
            fstreamV = new FileWriter("RssCollectionIndex/VocabularyFile.txt");
            outV = new BufferedWriter(fstreamV);
            fstreamP = new FileWriter("RssCollectionIndex/PostingFile.txt");
        ...
        {
            Logger.getLogger(RssIndex.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    private void makeDocumentsFile()
    {
        ...
        try 
        {
            fstream = new FileWriter("RssCollectionIndex/DocumentsFile.txt");  
    ...
    public SortedMap<String,Short> FileMainOperations(String path) throws FileNotFoundException, IOException, IllegalArgumentException, FeedException
    {
        ...
        File f = new File("RssCollectionIndex");
        f.mkdir();
